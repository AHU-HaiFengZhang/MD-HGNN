...
def generate_h_278(A):
    ...
    H = hstack([sparse_matrix_1, sparse_matrix_2])  # vatack 列拼接
...
def generate_edge_degree(A):
    ...
    # A43 = MO_.M43_edge_degree(A)
    # A44 = MO_.M44_edge_degree(A)
...
def generate_h_list(A):
    Mo = MotifGenerator()
    A = np.array(A)
    # l1 = Mo.M31_generator(A)
    l2 = Mo.M32_generator(A)
    # l3 = Mo.M41_generator(A)
    # l4 = Mo.M42_generator(A)
    # l5 = Mo.M43_generator(A)
    # l6 = Mo.M44_generator(A)
    l7 = Mo.M45_generator(A)
    l8 = Mo.M46_generator(A)
    # nested_list = merge_lists(l3, l4, l5, l6, l2, l7, l8)
    nested_list = merge_lists(l2, l7, l8)
    final_list = []
    ...
    for sublist in nested_list:
        ...
        if not is_subset:
            final_list.append(sublist)
    nested_list = [[int(v) for v in edge[0].split()] for edge in nested_list]
    final_list = [[int(v) for v in edge[0].split()] for edge in final_list]
    return nested_list, final_list


def generate_list(*lists):
    nested_list = merge_lists(lists)
    final_list = []
...
def list_d(list1, list2):
    merged_list = []
    for sublist1 in list1:
        is_contained = False
        for sublist2 in list2:
            if set(sublist1).issubset(sublist2):
                is_contained = True
                break
        if not is_contained:
            merged_list.append(sublist1)
    merged_list.extend(list2)
    return [list(map(int, sublist)) for sublist in merged_list]


def adjacency_matrix_to_adjacency_list(adj_matrix):  # K=1,返回包含自身的邻居节点的列表
    adjacency_list = []
