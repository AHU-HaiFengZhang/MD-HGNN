function W=M44_edge_degree(A)%A为邻接矩阵
    ...
    for i=1:length(b1)%非0元素的总数，每个非0元素都是一条边，可用（b1(i), b2(i)）确定；遍历每一条边
        ...
        lj2=setdiff(find(A(b2(i),:)),b1(i));%b2(i)邻居
         for j=1:length(lj1)
             for k=1:length(lj2)
                 if lj1(j)~=lj2(k)&&A(lj1(j),lj2(k))==0&&((A(lj1(j),b2(i))==1&&A(lj2(k),b1(i))==0)||(A(lj1(j),b2(i))==0&&A(lj2(k),b1(i))==1))
                    W(b1(i),b2(i))=W(b1(i),b2(i))+0.5;
                    W(lj1(j),b1(i))=W(lj1(j),b1(i))+0.5;
                    W(lj2(k),b2(i))=W(lj2(k),b2(i))+0.5;
                    W(lj1(j),b2(i))=W(lj1(j),b2(i))+0.5;
                    W(lj2(k),b1(i))=W(lj2(k),b1(i))+0.5;
                    W(lj1(j),lj2(k))=W(lj1(j),lj2(k))+0.5;
                 end
             end
         end
   end
